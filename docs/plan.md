...
4. Configure `BedrockModel` from `BEDROCK_MODEL_ID` and `AWS_REGION`, and register `get_local_time` as a deterministic tool.
...
- `BEDROCK_MODEL_ID`: a model ID enabled for the AWS account.
